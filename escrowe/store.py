"""Escrowe's own state, in one SQLite file: app settings (LLM choice, API
key, JWT secret), the configured source, and the audit log of every query.

There is no user table, no roles and no grants: access is decided by the
database escrowe connects to, never by escrowe.
"""
...
class Store:
    def __init__(self, path: Path | str):
        ...
        try:
            os.chmod(self.path, 0o600)    # may hold an API key: keep it to this user
        except OSError:
            pass
        with self._lock:
    ...
    def jwt_secret(self, configured: str | None) -> str:
        if configured:
            return configured
        secret = self.setting("jwt_secret")
        if not secret:
    ...
    def audit(self, **fields) -> int:
        ...
        return self._exec(f"INSERT INTO audit ({cols}) VALUES ({marks})", tuple(fields.values())).lastrowid
